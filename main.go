	...
	b.Buttons["read"] = widget.NewButton("Read", func() {
		...
		if err = Serial.Open(); err == nil {
		    ...
		    Try: func() {
					...
					data, err := Serial.Command(M.COMMAND_GET_REGISTER[:], M.GET_CONFIG[:])
					if len(data) == 10 {
						args := data[3:]
						...
						boot.entries["NETID"].SetText(fmt.Sprintf("%d", args[M.REGISTER_NETID[0]]))
						switch args[M.REGISTER_REG0[0]] & M.MASK_UART_BAUD {
						...
						switch args[M.REGISTER_REG0[0]] & M.MASK_UART_PARITY {
						case M.UART_8N1:
						...
						switch args[M.REGISTER_REG0[0]] & M.MASK_AIR_BAUD {
						case M.AIR_BAUD_300:
						...
						boot.selects["Channel"].SetText(fmt.Sprintf("%d", int(args[M.REGISTER_REG2[0]]) + 410 ))
						switch args[M.REGISTER_REG3[0]] & M.MASK_RSSI {
						case M.RSSI_ENABLE:
						...
						switch args[M.REGISTER_REG3[0]] & M.MASK_TRANSMISSION_MODE {
						...
						switch args[M.REGISTER_REG3[0]] & M.MASK_REPEATER {
						case M.REPEATER_ENABLE:
							boot.checks["Repeater"].SetChecked(true)
						case M.REPEATER_DISABLE:
							boot.checks["Repeater"].SetChecked(false)
						default:
							Throw("Unknown Repeater value!")
						}
						switch args[M.REGISTER_REG3[0]] & M.MASK_LBT {
						case M.LBT_ENABLE:
						...
						switch args[M.REGISTER_REG3[0]] & M.MASK_WOR_CONTROL {
						case M.WOR_TRANSMITTER:
						...
						boot.selects["WORCycle"].SetText(fmt.Sprintf("%d", (int(args[M.REGISTER_REG3[0]] & M.MASK_WOR_CYCLE) + 1) * 500 ))
						boot.SetState("Reading DONE")
					} else {
						logError("Reading", err)
					}
					data, err = Serial.Command(M.COMMAND_GET_REGISTER[:], M.GET_PRODUCT_INFO[:])
					if err == nil {
		...
		if err = Serial.Open(); err == nil {
		  TryCatchBlock {
		    Try: func() {
					...
					args[M.REGISTER_NETID[0]] = s2b(boot.entries["NETID"].Text)
					switch boot.selects["UARTRate"].Text {
					case "1200":
						args[M.REGISTER_REG0[0]] |= M.UART_BAUD_1200
					case "2400":
						args[M.REGISTER_REG0[0]] |= M.UART_BAUD_2400
					case "4800":
						args[M.REGISTER_REG0[0]] |= M.UART_BAUD_4800
					case "9600":
						args[M.REGISTER_REG0[0]] |= M.UART_BAUD_9600
					case "19200":
						args[M.REGISTER_REG0[0]] |= M.UART_BAUD_19200
					case "38400":
						args[M.REGISTER_REG0[0]] |= M.UART_BAUD_38400
					case "57600":
						args[M.REGISTER_REG0[0]] |= M.UART_BAUD_57600
					case "115200":
						args[M.REGISTER_REG0[0]] |= M.UART_BAUD_115200
					default:
						Throw("Unknown UARTRate value!")
					}
					switch boot.selects["UARTParityBit"].Text {
					case "8N1":
						args[M.REGISTER_REG0[0]] |= M.UART_8N1
					case "8O1":
						args[M.REGISTER_REG0[0]] |= M.UART_8O1
					case "8E1":
						args[M.REGISTER_REG0[0]] |= M.UART_8E1
					default:
						Throw("Unknown UARTParityBit value!")
					}
					switch boot.selects["WirelessRate"].Text {
					case "300":
						args[M.REGISTER_REG0[0]] |= M.AIR_BAUD_300
					case "1200":
						args[M.REGISTER_REG0[0]] |= M.AIR_BAUD_1200
					case "2400":
						args[M.REGISTER_REG0[0]] |= M.AIR_BAUD_2400
					case "4800":
						args[M.REGISTER_REG0[0]] |= M.AIR_BAUD_4800
					case "9600":
						args[M.REGISTER_REG0[0]] |= M.AIR_BAUD_9600
					case "19200":
						args[M.REGISTER_REG0[0]] |= M.AIR_BAUD_19200
					case "38400":
						args[M.REGISTER_REG0[0]] |= M.AIR_BAUD_38400
					case "62500":
						args[M.REGISTER_REG0[0]] |= M.AIR_BAUD_62500
					default:
						Throw("Unknown WirelessRate value!")
					}
					switch boot.selects["SubPacketLength"].Text {
					case "240":
						args[M.REGISTER_REG1[0]] |= M.SUB_PACKET_BYTES_240
					case "128":
						args[M.REGISTER_REG1[0]] |= M.SUB_PACKET_BYTES_128
					case "64":
						args[M.REGISTER_REG1[0]] |= M.SUB_PACKET_BYTES_64
					case "32":
						args[M.REGISTER_REG1[0]] |= M.SUB_PACKET_BYTES_32
					default:
						Throw("Unknown SubPacketLength value!")
					}
					if boot.checks["AmbientNoise"].Checked {
						args[M.REGISTER_REG1[0]] |= M.AMBIENT_NOISE_ENABLE
					}
					switch boot.selects["Power"].Text {
					case "33":
						args[M.REGISTER_REG1[0]] |= M.POWER_DBM_33
					case "30":
						args[M.REGISTER_REG1[0]] |= M.POWER_DBM_30
					case "27":
						args[M.REGISTER_REG1[0]] |= M.POWER_DBM_27
					case "24":
						args[M.REGISTER_REG1[0]] |= M.POWER_DBM_24
					default:
						Throw("Unknown Power value!")
					}
					...
					args[M.REGISTER_REG2[0]] = byte(channel)
					if boot.checks["RSSI"].Checked {
						args[M.REGISTER_REG3[0]] |= M.RSSI_ENABLE
					}
					switch boot.selects["TransmissionMode"].Text {
					case "Fixed point":
						args[M.REGISTER_REG3[0]] |= M.TRANSMISSION_MODE_FIXED
					case "Transparent":
						args[M.REGISTER_REG3[0]] |= M.TRANSMISSION_MODE_TRANSPARENT
					default:
						Throw("Unknown TransmissionMode value!")
					}
					if boot.checks["Repeater"].Checked {
						args[M.REGISTER_REG3[0]] |= M.REPEATER_ENABLE
					}
					if boot.checks["LBT"].Checked {
						args[M.REGISTER_REG3[0]] |= M.LBT_ENABLE
					}
					switch boot.selects["WOR"].Text {
					case "Transmitter":
						args[M.REGISTER_REG3[0]] |= M.WOR_TRANSMITTER
					case "Receiver":
						args[M.REGISTER_REG3[0]] |= M.WOR_RECEIVER
					default:
						Throw("Unknown WOR value!")
					}
					cycle := byte((getInt(boot.selects["WORCycle"].Text) / 500) - 1)
					if cycle > M.WOR_CYCLE_MS_4000 {
						Throw("Unknown WORCycle value!")
					}
					args[M.REGISTER_REG3[0]] |= cycle
					args[M.REGISTER_CRYPT_H[0]] = s2b(boot.entries["CryptH"].Text)
					args[M.REGISTER_CRYPT_L[0]] = s2b(boot.entries["CryptL"].Text)
					data, err := Serial.Command(M.COMMAND_SET_REGISTER[:], M.SET_CONFIG[:], args[:])
					if err != nil {
...
func createGUI() fyne.Window {
	...
	a := app.New()
	w := a.NewWindow("E22-400T33D Module Configuration Utility")
	w.SetContent(Show(w))
